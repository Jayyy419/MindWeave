      ...
      <Card className="border-amber-200/80 bg-[linear-gradient(145deg,#fff9ee_0%,#fffef7_45%,#f8f8ef_100%)] shadow-[0_20px_55px_-32px_rgba(74,53,21,0.4)]">
        <CardHeader className="pb-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <CardTitle className="text-xl text-stone-800">{entry.title || "Journal Entry"}</CardTitle>
        ...
        <CardContent className="space-y-5">
          <div className="relative rounded-xl border border-amber-100 bg-[repeating-linear-gradient(to_bottom,#fffef9_0px,#fffef9_30px,#ece7dc_31px)] p-4">
            ...
            <h3 className="mb-3 pl-8 text-sm font-medium text-stone-500">
              Saved Journal Replay
            </h3>
            <div className="space-y-3 pl-8 pr-2">
              {entry.chunks.map((chunk) => (
                <div key={chunk.id} className="rounded-md border border-amber-100/80 bg-white/55 p-3">
                  <p className="whitespace-pre-wrap text-[16px] leading-[31px] text-stone-800" style={{ fontFamily: "'Palatino Linotype', 'Book Antiqua', Palatino, serif" }}>
                    {chunk.userText}
                  </p>
                  {chunk.aiText && (
                    <p className="mt-2 whitespace-pre-wrap rounded-md border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm leading-relaxed text-indigo-900">
                      {chunk.aiText}
                    </p>
                  )}
                </div>
